...
public class Registry implements ContainerRegistry {
        ...
        try {
            ...
            if (containerAdapters.size() == 1) {
                ...
                for (Container container : containers) {
                    ...
                    if (isServiceLoaderClassAssignableFromAdapterImplClass(loader, adapterImplClassValue)) {
                        // copy-paste from LocalContainerRegistry
                        try {
                            return this.addContainer((Container) this.injector.inject(
                                new ContainerImpl(definition.getContainerName(),
                                                  (DeployableContainer) loader.onlyOne(DeployableContainer.class),
                                                  definition)));
                        } catch (Exception var4) {
                            throw new ContainerCreationException(
                                "Could not create Container " + definition.getContainerName(), var4);
                        }
                    }
                }
                if (dcService == null) {
    ...
    private boolean isServiceLoaderClassAssignableFromAdapterImplClass(ServiceLoader loader,
        String adapterImplClassValue) {
