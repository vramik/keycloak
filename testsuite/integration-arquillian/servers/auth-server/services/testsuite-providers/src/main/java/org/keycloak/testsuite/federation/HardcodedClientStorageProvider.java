...
public class HardcodedClientStorageProvider implements ClientStorageProvider, ClientLookupProvider {
    ...
    public Map<String, ClientScopeModel> getClientScopes(ClientModel client, boolean defaultScope, boolean filterByProtocol) {
        if (defaultScope) {
                ClientScopeModel rolesScope = KeycloakModelUtils.getClientScopeByName(client.getRealm(), OIDCLoginProtocolFactory.ROLES_SCOPE);
                ClientScopeModel webOriginsScope = KeycloakModelUtils.getClientScopeByName(client.getRealm(), OIDCLoginProtocolFactory.WEB_ORIGINS_SCOPE);
                return Arrays.asList(rolesScope, webOriginsScope)
                        .stream()
                        .collect(Collectors.toMap(ClientScopeModel::getName, clientScope -> clientScope));

            } else {
                ClientScopeModel offlineScope = KeycloakModelUtils.getClientScopeByName(client.getRealm(), "offline_access");
