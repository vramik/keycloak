...
public class MapRealmProvider implements RealmProvider {
    ...
    public MapRealmProvider(KeycloakSession session, MapStorage<String, MapRealmEntity, RealmModel> realmStore) {
        ...
        this.tx = new MapKeycloakTransaction<>(realmStore);
        session.getTransactionManager().enlist(tx);
    }

    private Function<MapRealmEntity, RealmModel> entityToAdapterFunc() {
        // Clone entity before returning back, to avoid giving away a reference to the live object to the caller

        return origEntity -> new MapRealmAdapter(session, registerEntityForChanges(origEntity));
    }
    ...
    public RealmModel createRealm(String id, String name) {
        ...
        final String entityId = id == null ? KeycloakModelUtils.generateId() : id;

        LOG.tracef("createRealm(%s, %s)%s", id, name, getShortStackTrace());

        MapRealmEntity entity = new MapRealmEntity(entityId);
        entity.setName(name);
        if (tx.read(entity.getId()) != null) {
            throw new ModelDuplicateException("Realm exists: " + entityId);
        }
    ...
    public Stream<RealmModel> getRealmsStream() {
        return tx.getUpdatedNotRemoved(realmStore.createCriteriaBuilder())
                .map(entityToAdapterFunc())
                .sorted(RealmModel.COMPARE_BY_NAME);
    }

    @Override
    public Stream<RealmModel> getRealmsWithProviderTypeStream(Class<?> type) {
        return StreamUtils.leftInnerJoinStream(tx.getUpdatedNotRemoved(realmStore.createCriteriaBuilder()), MapRealmEntity::getComponents) // create pair of realm and its components
                .filter(pair -> Objects.equals(type.toString(), pair.getV().getProviderType())) // filter components based on type
                .map(Pair::getK) // get remaining realms
                .map(entityToAdapterFunc())
                .sorted(RealmModel.COMPARE_BY_NAME);
    }
    ...
    public void removeExpiredClientInitialAccess() {
        ModelCriteriaBuilder<RealmModel> mcb = realmStore.createCriteriaBuilder()
                .compare(SearchableFields.CLIENT_INITIAL_ACCESS, Operator.EQ, true);
